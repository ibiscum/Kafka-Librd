...

{
    ...
    my $kafka = Kafka::Librd->new(
    	    Kafka::Librd::RD_KAFKA_PRODUCER,
    	    {
    	        ...
    	        # this constraints the message size a well as topic name size
    ...
    $err = Kafka::Librd::Error::last_error();
    is $value, -1, "too large message produce returns error";
    is $err, Kafka::Librd::RD_KAFKA_RESP_ERR_MSG_SIZE_TOO_LARGE, "last error returns too large message size";

    # same size constraints apply to topic creation call
    $topic = $kafka->topic($large_message, {});
    $err = Kafka::Librd::Error::last_error();
    is $topic, undef, "invalid arguments, topic undefined";
    is $err, Kafka::Librd::RD_KAFKA_RESP_ERR__INVALID_ARG, "last error returns invalid arguments";
}
